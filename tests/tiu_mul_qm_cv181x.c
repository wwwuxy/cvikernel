...
int main() {
    printf("Running test: cvkcv181x_tiu_mul_qm...\n");
    ...
    // Configuration mul_qm operation parameters
    cvk_tiu_mul_qm_param_t mul_qm_param;
    memset(&mul_qm_param, 0, sizeof(cvk_tiu_mul_qm_param_t));

    mul_qm_param.a = a;
    mul_qm_param.b = b;
    mul_qm_param.res_low = res_low;
    mul_qm_param.res_high = res_high;  // Optional, set to NULL to test the case with only res_low
    mul_qm_param.b_is_const = 0;       // b Not a constant
    mul_qm_param.relu_enable = 0;       // Disable ReLU
    mul_qm_param.rshift_bits = 0;       // Do not shift right
    mul_qm_param.multiplier = 1;        // Set the quantization multiplier to 1, the result is a * b
    mul_qm_param.layer_id = 0;

    printf("Calling cvkcv181x_tiu_mul_qm...\n");
    // Call mul_qm function
    cvkcv181x_tiu_mul_qm(&ctx, &mul_qm_param);
