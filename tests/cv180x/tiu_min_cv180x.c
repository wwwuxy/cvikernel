...
void test_tiu_min() {
    ...
    min_param.b_is_const = 0;   // 使用张量而非常量
    ...
    printf("示例结果:\n");
    for (int i = 0; i < 5; i++) {
        int8_t min_val = (sample_input1[i] < sample_input2[i]) ? 
                        sample_input1[i] : sample_input2[i];
        printf("input1[%d]=%d, input2[%d]=%d, min=%d\n", 
               i, sample_input1[i], i, sample_input2[i], min_val);
    }
#endif // CV180X_USE_REAL_IMPL
...
void test_tiu_min_constant() {
    ...
    min_param.b_is_const = 1;   // 使用常量
    min_param.b_const.val = 5;
    min_param.b_const.is_signed = 1;
    min_param.layer_id = 0;
    ...
    printf("示例结果:\n");
    for (int i = 0; i < 10; i++) {
        int8_t min_val = (sample_input[i] < constant) ? 
                        sample_input[i] : constant;
        printf("input[%d]=%d, constant=%d, min=%d\n", 
               i, sample_input[i], constant, min_val);
    }
#endif // CV180X_USE_REAL_IMPL
