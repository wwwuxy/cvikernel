...
void test_tiu_max() {
    ...
    max_param.max = tl_output;
    max_param.a = tl_input1;
    max_param.b_is_const = 0;   // 使用张量而非常量
    ...
    printf("示例结果:\n");
    for (int i = 0; i < 5; i++) {
        int8_t max_val = (sample_input1[i] > sample_input2[i]) ? 
...
void test_tiu_max_constant() {
    ...
    memset(&max_param, 0, sizeof(max_param));
    max_param.max = tl_output;
    max_param.a = tl_input;
    max_param.b_is_const = 1;   // 使用常量
    max_param.b_const.val = 5;
    ...
    printf("示例结果:\n");
    for (int i = 0; i < 10; i++) {
        int8_t max_val = (sample_input[i] > constant) ? 
