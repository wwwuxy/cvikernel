...
cvk_tl_t* create_tensor(int n, int c, int h, int w,
    ...
    tensor->start_address = (uintptr_t)malloc(count * sizeof(int8_t));
    return tensor;
}
...
// Fill the int8 data with a simple pattern
// for debugging and verification
static void fill_data_int8(cvk_tl_t* tensor, int8_t base_value) {
    int8_t* ptr = (int8_t*)tensor->start_address;
    int N = tensor->shape.n;
    int C = tensor->shape.c;
    int H = tensor->shape.h;
    int W = tensor->shape.w;

    // Fill each element with (base_value + linear_index) & 0xFF
    // just to have a distinct pattern per tensor
...
static void ref_xor_int8(const cvk_tl_t* a,
                         const cvk_tl_t* b,
                         int8_t* ref_output) {
    const int8_t* a_data = (int8_t*)a->start_address;
    const int8_t* b_data = (int8_t*)b->start_address;
...
static void compare_xor_int8(const cvk_tl_t* res,
    ...
    const int8_t* hw_data = (int8_t*)res->start_address;
    int N = res->shape.n;
    int C = res->shape.c;
    int H = res->shape.h;
    int W = res->shape.w;
...
static void ref_xor_int16(const cvk_tl_t* a_low, const cvk_tl_t* a_high,
    ...
    const int8_t* al = (int8_t*)a_low->start_address;
    const int8_t* ah = (int8_t*)a_high->start_address;
    const int8_t* bl = (int8_t*)b_low->start_address;
    const int8_t* bh = (int8_t*)b_high->start_address;
...
static void compare_xor_int16(const cvk_tl_t* res_low, const cvk_tl_t* res_high,
    ...
    const int8_t* hl = (int8_t*)res_low->start_address;
    const int8_t* hh = (int8_t*)res_high->start_address;
...
int main() {
    ...
    // res_int8, res_low_int16, res_high_int16 will be overwritten, so no fill needed
    ...
    // Clean up
    cvkcv181x_lmem_free_tensor(&ctx, (void*)a_int8->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)b_int8->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)res_int8->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)a_low_int16->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)a_high_int16->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)b_low_int16->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)b_high_int16->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)res_low_int16->start_address);
    cvkcv181x_lmem_free_tensor(&ctx, (void*)res_high_int16->start_address);
