...
int main() {
    printf("Running test: cvkcv181x_tiu_mul...\n");
    ...
    // Configure mul operation parameters
    cvk_tiu_mul_param_t mul_param;
    memset(&mul_param, 0, sizeof(cvk_tiu_mul_param_t));

    mul_param.a = a;
    mul_param.b = b;
    mul_param.res_low = res_low;
    mul_param.res_high = res_high;  // Optional, set to NULL to test the case with only res_low
    mul_param.b_is_const = 0;       // b Not a constant
    mul_param.relu_enable = 0;       // Disable ReLU
    mul_param.rshift_bits = 0;       // Do not shift right
    mul_param.layer_id = 0;

    printf("Calling cvkcv181x_tiu_mul...\n");
    // Call mul function
    cvkcv181x_tiu_mul(&ctx, &mul_param);
    ...
    for (size_t i = 0; i < total_size; ++i) {
        ...
        assert(res_low_data[i] == 6);  // 2 * 3 should be equal to 6
    }
    ...
    for (size_t i = 0; i < high_total_size; ++i) {
        ...
        assert(res_high_data[i] == 6);  // 2 * 3 should be equal to 6
    }
